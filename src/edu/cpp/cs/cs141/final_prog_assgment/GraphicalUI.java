...
public class GraphicalUI extends UserInterface {
	/**
	 * JFrame that contains all Objects used
	 */
	private JFrame frame;
	/**
	 * JPanel that contains the 9x9 grid of ImageIcons
	 */
	private JPanel mapPanel;
	/**
	 * JPanel with Main Menu options
	 */
	private JPanel menuPanel;
	/**
	 * contains all of the labels to be shown in GUI with same coordinates as
	 * formatMap()
	 */
	private JLabel[][] Labelmap = new JLabel[9][9];
	/**
	 * Stores the last map that was generated by
	 * {@link #printMap(String[], char, boolean, boolean)} , Used for efficency
	 * rendering images
	 */
	private char[][] oldmap = new char[9][9];
	/**
	 * Display Help to User, and Provide Tips occasionally
	 */
	private JTextArea txtArea;
	/**
	 * Option that Player chose in the main menu
	 */
	private int menuOption;
	/**
	 * Concurrency Latch to Ensure User selects choice before continuing
	 */
	private CountDownLatch latch;
	/**
	 * The Current help Page that is showing
	 */
	private int pageNumber;
	/**
	 * JPanel containing all of the JButtons a User can take, Also allows entry into
	 * debug mode
	 */
	private JPanel pickTurnPanel;
	/**
	 * Option that last User picked when Picking Turn.
	 */
	private int pickTurnOption;
	/**
	 * JPanel containing all of the JButtons to let the User decide a Direction
	 */
	private JPanel pickDirectionPanel;
	/**
	 * Direction that User last Picked
	 */
	private char pickDirectionOption;
	private JSeparator separator1;
	/**
	 * Icons resized to fit monitor, used to Display
	 */
	private ImageIcon[] icons = new ImageIcon[9];
	/**
	 * Base Icons taken directly from File. Must be resized before use
	 */
	private ImageIcon[] baseIcons = new ImageIcon[9];
	/**
	 * Vertical Pixel Height of a Single Image, to be used in {@link #reloadMap()}
	 */
	private int pixelCount = 20;
	/**
	 * Output to Speakers used for Background Music
	 */
	...
	/**
	 * Decides the "mode" the GUi is in 0 = Anything Else 1 = Pick Turn 2 =
	 * Direction Pick
	 */
	private volatile int mode = 0;
	/**
	 * Skeleton Bar for GUI only actions
	 */
	private JMenuBar menuBar;
	/**
	 * Button to mute or unmute music
	 */
	private JCheckBoxMenuItem chckbxmntmMuteMusic;
	/**
	 * Button to Open Save Directory for easy access
	 */
	private JMenuItem mntmOpenFolder;
	/**
	 * Button to change the Size of the Map Tokens
	 */
	private JMenuItem mntmChangeImageResolution;
	/**
	 * AudioStream that {@link #soundLine} will play
	 */
	private AudioInputStream audioStream;
	/**
	 * Holds if the last time the map was rendered, if Shooting was possible
	 */
	private boolean canShoot;
	/**
	 * Holds if the last time the map was rendered, if Looking was possible
	 */
	private boolean canLook;

	/**
	 * Contruct all GUI elements, What they do, Start Music, and Display Panel
	 * Starts Program without a Button Panel, allows GameEngine to decide what to
	 * showfirst
	 */
	public GraphicalUI() {
		...
		frame.setIconImage(Toolkit.getDefaultToolkit()
				.getImage(GraphicalUI.class.getResource("/edu/cpp/cs/cs141/final_prog_assgment/spy.png")));
		...
		frame.getContentPane().add(scrollPane, "cell 0 3,grow,wmin 10");
		...
		scheduler = new Timer(1000, new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				Labelmap[random.nextInt(9)][random.nextInt(9)].setIcon(icons[random.nextInt(8)]);
	...
	/**
	 * Gui Constructor SubMethod to assemble MenuBar and its Buttons
	 */
	...
	/**
	 * Create the SourceDataLine, and start a thread to loop the background Song
	 */
		...
		if (mapPanel.isVisible()) {
			...
			for (int x = 0; x < 9; x++) {
				for (int y = 0; y < 9; y++) {
					switch (oldmap[x][y]) {
					case 'P':
						Labelmap[x][y].setIcon(icons[0]);// http://katonahchamber.org/wp/wp-content/uploads/2017/05/Spy_icon.png
						break;
					case 'N':
	...
	/**
	 * In GUI Only, display Quick Notes about the UI
	 */
	@Override
	public void welcomeMessage() {
		JOptionPane.showMessageDialog(frame,
				"Debug Mode can be enabled by clicking the Question when Picking a Turn\nMake sure to change the Image resolution custom to your monitor\nIf you want to play the console version,\nappend -cui at the end of a cmd activation command",
	...
	public void printShotResult(boolean result) {
		...
		JOptionPane.showMessageDialog(frame, message, "Shot Result", JOptionPane.INFORMATION_MESSAGE);
	}

	/**
	 * This inner class is for the GUI only, to request the Resolution from the
	 * User, and verify the number before closing
	 */
	class ResolutionQueryDialog extends JDialog {
		 ...
		 * 
		 * @param window
		 *            JFrame to render Over
		 */
		public ResolutionQueryDialog(JFrame window) {
			super(window, true);
			setTitle("Resolution Size Query");

			pixelField = new JSpinner(new SpinnerNumberModel(20, 10, 150, 10));

			String msgString1 = "What length do you want the Map Tokens to be?\nEnter Size in pixel count, range from (10-150)\nImproper Numbers will be Ignored";
			...
			btn1.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					try {
						pixelField.commitEdit();// Not Throwing Errors
						saveable = true;
					} catch (ParseException pe) {
						((JSpinner.NumberEditor) pixelField.getEditor()).getTextField()
								.setValue(((int) pixelField.getValue()));
					}
	...
	/**
	 * This inner class is for the GUI only to Request the Filename of the new Save
	 * file, and verify the Path before closing.
	 */
	...
	public String queryLoadFileName(String[] saves) {
		...
		String stored;
		do {
			stored = (String) JOptionPane.showInputDialog(frame, "Which Save File would you like to Load?", "Load Save",
					JOptionPane.QUESTION_MESSAGE, null, saves, saves[0]);
			if (stored == null) {
				JOptionPane.showMessageDialog(frame, "Please make a Selection", "Bad Input", JOptionPane.ERROR_MESSAGE);
			}
		} while (stored == null);
		return stored;
	}
	...
	public boolean offerDifficulty() {
		...
		if (option == 0) {
			...
			return false; // Also if User tries to quit
		}
	}
